...
# ALLOWED_MIME_TYPES = ["image/jpeg", "image/jpg", "image/png", "image/JPEG", "image/JFIF"]
ALLOWED_MIME_TYPES = ["JPG", "PNG", "JPEG"]
...
async def add_car(request: Request, car_id: int,file: UploadFile,db: Session = Depends(get_db)):
    # validate file
    # Initialize the magic library for MIME type detection
    mime = magic.Magic()
    ...
    # Check if any word from mime_type is in the input string
    if not any(word in detected_mime_type for word in ALLOWED_MIME_TYPES):
